...
<Meta title="hooks/useOptionFieldProps" />

# useOptionFieldProps(field: OptionField)
...
```ts
type OptionField = BooleanField | NumberField | StringField;
```
...
```tsx
const SelectInput = <Option, Field extends OptionField>({
  field,
  label,
  getValue,
  getLabel,
